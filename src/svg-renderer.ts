...
export class SVGRenderer {
  private readonly defaultOptions: Required<SVGOptions> = {
    width: 500,
    height: 300,
    padding: 40,
    xMin: 0,
  ...
  private scaleX(x: number): number {
    const { xMin, xMax, width, padding } = this.options as Required<SVGOptions>;
    return padding + ((x - xMin) / (xMax - xMin)) * (width - 2 * padding);
  }

  private scaleY(y: number): number {
    const { yMin, yMax, height, padding } = this.options as Required<SVGOptions>;
  ...
  private getShapePoints(shape: any): Array<[number, number]> {
    ...
    } else if (shape instanceof Sigmoid) {
    ...
    } else if (shape instanceof Constant) {
      // Constant is a horizontal line
      const y = shape.fuzzify();
      points.push([xMin, y]);
      points.push([xMax, y]);
    } else if (shape instanceof FuzzyFunction) {
